...
def get_resources(profile, region, report_date, three_month,
    ...
    logger.debug(f'   Number of unattached elastic IPs: {ip_count}')
    ...
    logger.debug(f'   Number of unattached volumes: {unatt_volume_count}')
    logger.debug(f'   Number of valid unattached volumes: {valid_volume_count}')
    ...
    df_unami, unused_image_count = euna.get_unused_images(ec2, account_name, account_number, region_name,
                                                          valid_old, three_month, df_unami, logger)
    logger.debug(f'   Number of valid unused images: {unused_image_count}')
    ...
    df_rdssnaps, db_snapshot_count, aurora_snapshot_count = rsnap.get_old_rds_snaps(rds, account_name, account_number,
                                                                                    ...
                                                                                    df_rdssnaps, logger)
    logger.debug(f'   Number of valid old RDS snapshots: {db_snapshot_count}')
    logger.debug(f'   Number of valid old Aurora snapshots: {aurora_snapshot_count}')
